...
func TestPostgresApplicationRepository(t *testing.T) {
	...
	if found.ID != application.ID || found.Product.ID != product.ID {
		t.Fatalf("FindByID() = %+v, want preloaded product", found)
	}
...
func sqliteDB(t *testing.T) *gorm.DB {
	...
	if err := db.AutoMigrate(&models.Product{}, &models.Application{}); err != nil {
